"""
Simulator Module for UAE Pulse Dashboard (Optimizer Upgrade)
- Existing KPI methods preserved
- simulate_campaign preserved
- NEW: recommend_campaign() to choose discount/margin_floor/days for best ROI
"""
...
class Simulator:
    ...
    def _get_sku_column(self, df):
        return self._find_column(df, ['sku', 'SKU', 'sku_id', 'product_id', 'ProductID', 'item_id'])

    def _get_cost_column(self, df):
        return self._find_column(df, ['unit_cost_aed', 'cost_aed', 'cost', 'unit_cost', 'purchase_price'])
    ...
    def calculate_overall_kpis(self, sales_df, products_df):
        ...
        try:
            ...
            if sku_sales and sku_prod and cost_col:
            ...
            kpis['total_orders'] = int(merged[order_col].nunique()) if order_col else int(len(merged))
            ...
            kpis['profit_margin_pct'] = (kpis['total_profit'] / kpis['total_revenue'] * 100) if kpis['total_revenue'] else 0
            ...
            return {
                ...
                'total_units': 0, 'avg_order_value': 0, 'profit_margin_pct': 0
            }
    ...
    def simulate_campaign(
        ...
        """
        Forward simulation: given discount/margin_floor/days -> outputs.
        We'll use this inside recommend_campaign() many times.
        """
        try:
            merged = sales_df.copy()

            # merge stores => bring city/channel for filtering
            ...
            if stores_df is not None and store_sales and store_stores:
                ssub_cols = [store_stores]
                if city_col: ssub_cols.append(city_col)
                if channel_col: ssub_cols.append(channel_col)
                ssub = stores_df[ssub_cols].copy()
                ssub.columns = ['_store'] + ssub_cols[1:]
                merged['_store'] = merged[store_sales]
                merged = merged.merge(ssub, on='_store', how='left')

            # merge products => cost + category for filtering
            sku_sales = self._get_sku_column(sales_df)
            ...
            if products_df is not None and sku_sales and sku_prod:
                psub_cols = [sku_prod]
                if cost_col: psub_cols.append(cost_col)
                if cat_col: psub_cols.append(cat_col)
                psub = products_df[psub_cols].copy()
                new_cols = ['_sku']
                if cost_col: new_cols.append('_cost')
                if cat_col: new_cols.append('category')
            ...
            # Lift
            demand_lift_pct = float(np.clip(discount_pct * elasticity, -50, 400))
            expected_units = baseline_units * (1 + demand_lift_pct / 100)

            avg_price = float(merged['_price'].mean()) if len(merged) else 0
            avg_cost = float(merged['_cost'].mean()) if len(merged) else 0
